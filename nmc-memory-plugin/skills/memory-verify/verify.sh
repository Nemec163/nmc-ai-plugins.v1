...
CANON_VERIFY_CLI="$PLUGIN_ROOT/../packages/memory-canon/lib/verify-cli.js"
...
main() {
  local memory_root meta_dir manifest_file updated_at today warning_count meta_rel
  ...
  warning_count="$(node "$CANON_VERIFY_CLI" "$memory_root" "$updated_at" "$today")"
