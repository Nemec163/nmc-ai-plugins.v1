...
ADAPTER_ROOT="$REPO_ROOT/packages/adapter-openclaw"
MANIFEST_FILE="$ADAPTER_ROOT/openclaw.plugin.json"
PACKAGE_FILE="$ADAPTER_ROOT/package.json"
ENTRY_FILE="$ADAPTER_ROOT/plugin.js"
...
VERIFY_SCRIPT="$ADAPTER_ROOT/skills/memory-verify/verify.sh"
STATUS_SCRIPT="$ADAPTER_ROOT/skills/memory-status/status.sh"
ONBOARD_SCRIPT="$ADAPTER_ROOT/skills/memory-onboard-agent/onboard.sh"
PIPELINE_SCRIPT="$ADAPTER_ROOT/skills/memory-pipeline/pipeline.sh"
RETENTION_SCRIPT="$ADAPTER_ROOT/skills/memory-retention/retention.sh"
TEMPLATE_ROOT="$ADAPTER_ROOT/templates/workspace-memory"
SETUP_SCRIPT="$ADAPTER_ROOT/lib/setup-cli.js"
...
link_workspace_packages() {
  ...
  local package_layout_root="$2"

  mkdir -p "$node_modules_root" "$package_layout_root"

  "$NODE_BIN" - "$node_modules_root" "$package_layout_root" "$REPO_ROOT/packages" <<'EOF'
const fs = require("fs");
const path = require("path");

const [nodeModulesRoot, packageLayoutRoot, packagesRoot] = process.argv.slice(2);
...
for (const entry of fs.readdirSync(packagesRoot)) {
  ...
  const moduleTarget = path.join(nodeModulesRoot, ...manifest.name.split("/"));
  const layoutTarget = path.join(packageLayoutRoot, entry);

  fs.mkdirSync(path.dirname(moduleTarget), { recursive: true });

  try {
    fs.symlinkSync(packageDir, moduleTarget, "dir");
  } catch (error) {
  ...
  try {
    fs.symlinkSync(packageDir, layoutTarget, "dir");
  } catch (error) {
    if (error.code !== "EEXIST") {
      throw error;
    }
...
test_packaging_files() {
  ...
  if [ "$(json_query "$MANIFEST_FILE" 'id')" = "memoryos-openclaw" ] && \
  ...
  if [ "$(json_query "$PACKAGE_FILE" 'name')" = "adapter-openclaw" ] && \
     [ "$(json_query "$PACKAGE_FILE" 'main')" = "./index.js" ] && \
     [ "$(json_query "$PACKAGE_FILE" 'openclaw.extensions.0')" = "./plugin.js" ]; then
  ...
  run_and_capture "$NODE_BIN" - "$ADAPTER_ROOT" <<'EOF'
const assert = require("node:assert/strict");
const fs = require("node:fs");
const path = require("node:path");

const adapterRoot = process.argv[2];
const {
  INSTALL_SURFACES,
...
EOF
  ...
  if [ "$LAST_EXIT_CODE" -eq 0 ]; then
    pass "adapter install metadata matches the adapter-owned source of truth"
  else
    fail "adapter install metadata matches the adapter-owned source of truth" "Expected install metadata to match packages/adapter-openclaw/lib/install-surface.js"
    printf '  stderr: %s\n' "$(cat "$LAST_STDERR")"
  fi
}
...
test_skill_frontmatter() {
  ...
  for file in "$ADAPTER_ROOT"/skills/*/SKILL.md; do
...
test_packaged_artifact_install_smoke() {
  local node_bin npm_bin tool_dir artifact_root extract_root install_root state_dir workspace_root
  local config_path package_name packaged_root packaged_setup packaged_onboard
  local packaged_pipeline packaged_manifest packaged_package packaged_plugin
  local packaged_memory_template packaged_system_template packaged_memory_root
  local packaged_control_plane_cli packaged_gateway_cli packaged_control_plane_root
  local packaged_gateway_root installed_root installed_setup installed_control_plane_cli
  local installed_gateway_cli

  print_case "TEST" "packed adapter-openclaw artifact stays bootstrappable after extract"
  ...
  mkdir -p "$artifact_root" "$extract_root" "$install_root" "$state_dir"

  run_and_capture_in_dir "$artifact_root" env PATH="$tool_dir:$PATH" "$npm_bin" pack "$ADAPTER_ROOT" --silent
  ...
  packaged_package="$packaged_root/package.json"
  packaged_plugin="$packaged_root/plugin.js"
  packaged_setup="$packaged_root/lib/setup-cli.js"
  ...
  packaged_memory_template="$packaged_root/templates/workspace-memory"
  packaged_system_template="$packaged_root/templates/workspace-system"
  packaged_control_plane_root="$packaged_root/control-plane"
  packaged_gateway_root="$packaged_root/memory-os-gateway"
  ...
  if [ -f "$packaged_manifest" ] && \
     ...
     [ -f "$packaged_onboard" ] && \
     [ -f "$packaged_pipeline" ] && \
     [ -d "$packaged_control_plane_root" ] && \
     [ -d "$packaged_gateway_root" ] && \
     [ -d "$packaged_root/memory-pipeline" ] && \
     [ -d "$packaged_root/memory-contracts" ] && \
     [ -d "$packaged_root/memory-canon" ] && \
     [ -d "$packaged_root/memory-agents" ] && \
     [ -d "$packaged_root/memory-workspace" ] && \
     [ -d "$packaged_root/memory-scripts" ] && \
     [ -d "$packaged_root/memory-maintainer" ] && \
     [ -d "$packaged_root/memory-os-runtime" ] && \
     [ -d "$packaged_memory_template" ] && \
     [ -d "$packaged_system_template" ]; then
    pass "packed artifact bundles adapter install surface"
  else
    fail "packed artifact bundles adapter install surface" "Expected adapter install surface under $packaged_root"
    return
  fi

  if [ "$(json_query "$packaged_manifest" 'skills.0')" = "./skills" ] && \
     [ "$(json_query "$packaged_package" 'openclaw.extensions.0')" = "./plugin.js" ]; then
    pass "packed artifact retains adapter-owned install metadata"
  else
    fail "packed artifact retains adapter-owned install metadata" "Expected packaged adapter install metadata under $packaged_root"
    return
  fi
  ...
  run_and_capture env PATH="$tool_dir:$PATH" "$node_bin" -e 'const assert = require("node:assert/strict"); const plugin = require(process.argv[1]); assert.equal(plugin.id, "memoryos-openclaw"); assert.equal(plugin.name, "MemoryOS OpenClaw Adapter");' "$packaged_plugin"

  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
    fail "packed artifact plugin entry exports expected metadata" "Expected 0, got $LAST_EXIT_CODE"
    printf '  stderr: %s\n' "$(cat "$LAST_STDERR")"
    return
  fi

  pass "packed artifact plugin entry exports expected metadata"

  run_and_capture env PATH="$tool_dir:$PATH" "$node_bin" "$packaged_control_plane_cli" health \
    --memory-root "$packaged_memory_root" \
    --system-root "$workspace_root/system"
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
    fail "packed artifact control-plane wrapper cli" "Expected 0, got $LAST_EXIT_CODE"
    printf '  stderr: %s\n' "$(cat "$LAST_STDERR")"
    return
  fi

  run_and_capture env PATH="$tool_dir:$PATH" "$node_bin" "$packaged_gateway_cli" status \
    --memory-root "$packaged_memory_root"
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
    fail "packed artifact gateway wrapper cli" "Expected 0, got $LAST_EXIT_CODE"
    printf '  stderr: %s\n' "$(cat "$LAST_STDERR")"
    return
  fi

  pass "packed artifact wrapper clis execute after extract"
  ...
  run_and_capture_in_dir "$install_root" env PATH="$tool_dir:$PATH" "$npm_bin" init -y
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
    fail "packed artifact clean npm init" "Expected 0, got $LAST_EXIT_CODE"
    printf '  stderr: %s\n' "$(cat "$LAST_STDERR")"
    return
  fi

  run_and_capture_in_dir "$install_root" env PATH="$tool_dir:$PATH" "$npm_bin" install "$artifact_root/$package_name"
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
    fail "packed artifact clean npm install" "Expected 0, got $LAST_EXIT_CODE"
    printf '  stderr: %s\n' "$(cat "$LAST_STDERR")"
    return
  fi

  installed_root="$install_root/node_modules/adapter-openclaw"
  installed_setup="$installed_root/lib/setup-cli.js"
  installed_control_plane_cli="$installed_root/bin/memory-control-plane.js"
  installed_gateway_cli="$installed_root/bin/memory-os-gateway.js"

  if [ -f "$installed_setup" ] && \
     [ -f "$installed_control_plane_cli" ] && \
     [ -f "$installed_gateway_cli" ] && \
     [ -d "$installed_root/control-plane" ] && \
     [ -d "$installed_root/memory-os-gateway" ]; then
    pass "packed artifact installs cleanly in a temp project"
  else
    fail "packed artifact installs cleanly in a temp project" "Expected installed wrapper paths under $installed_root"
    return
  fi
}
...
test_runtime_auto_bootstrap_without_state_dir() {
  ...
  plugin_root="$runtime_root/plugin"
  mkdir -p "$runtime_root"

  print_case "TEST" "runtime entrypoint tolerates missing state-dir hints"

  cp -R "$ADAPTER_ROOT" "$plugin_root"
  link_workspace_packages "$plugin_root/node_modules" "$runtime_root"

  run_and_capture env -u OPENCLAW_STATE_DIR "$NODE_BIN" - "$plugin_root" <<'EOF'
const fs = require("fs");
const path = require("path");
