...
```
nmc-memory-plugin/
  plugin.json                          # manifest: name, version, skills list
  skills/
    memory-extract/
      SKILL.md
    memory-curate/
...
1. `openclaw plugin install nmc-memory-plugin`
2. Плагин копирует `templates/workspace-memory/` в workspace пользователя.
