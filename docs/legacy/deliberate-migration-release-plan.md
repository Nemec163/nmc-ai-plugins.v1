...
It is a planning artifact for the current direct-install posture of `MemoryOS.v1`. It does not change workspace layout, canon format, or control-plane authority.
...
| --- | --- | --- |
| `MemoryOS.v1` core packages | Autonomous memory contracts, canon, runtime, gateway, pipeline, scripts, and control-plane surfaces | Treat as the primary product boundary and keep them engine-agnostic |
| `packages/adapter-openclaw` | Direct OpenClaw install, setup, auto-bootstrap, bundled skills, and plugin packaging surface | Treat as the supported production OpenClaw adapter/plugin surface for MemoryOS.v1 |
| `nmc-memory-plugin` | Retired legacy shell | Keep only as a historical identifier in roadmap and release-qualification metadata; do not restore it as an install path |
| `packages/control-plane` | Supported read-only operator SDK/CLI | Treat as the supported operator surface for migration-release automation and operator workflows |
...
- reintroducing `nmc-memory-plugin` as an install, setup, or packaging surface
...
1. Promote `packages/adapter-openclaw` as the supported production install/setup shell for OpenClaw packaging, setup, bootstrap, and bundled skills.
...
4. Keep `nmc-memory-plugin` retired and out of the repository while preserving the direct adapter install surface and release metadata that record that retirement.
...
- complete on `2026-03-19` by recording five pending cutover gates in release qualification and docs:
  ...
  - `shipped-artifact-layout`: legacy mirrors may still reference `~/.openclaw/extensions/nmc-memory-plugin/` even though the production adapter now installs under `~/.openclaw/extensions/memoryos-openclaw/`
