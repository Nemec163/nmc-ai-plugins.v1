...
function usage() {
  console.error(
    'Usage: run-phase.js --phase <extract|curate> --date <YYYY-MM-DD> --memory-root <path> --role-id <id> --workspace-dir <path> --system-root <path> --shared-skills-root <path> --install-date <date> --llm-runner <cmd> [--source-glob <hint>]'
  );
}
...
function runClaudePhase(flags) {
  ...
  const intake = getClaudeRoleBundleIntake({
    roleId,
    workspaceDir,
    systemRoot,
    memoryRoot,
